...
RUN apk update && \
    apk upgrade && \
    apk add glib-dev gcc git make musl-dev protobuf-c-dev openssl-dev krb5-dev && \
...
ARG POSTGRES_VERSION
RUN apk add glib libgcc libstdc++ musl musl-utils krb5 openssl protobuf protobuf-c && \
